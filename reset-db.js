...
async function reset() {
  ...
  await db.query("ALTER TABLE customer AUTO_INCREMENT = 1");
  ...
  if (statusCol.length === 0) {
    await db.query(
      "ALTER TABLE issue ADD COLUMN status ENUM('open','in_progress','resolved','escalated') NOT NULL DEFAULT 'open'",
    );
  ...
  // seed 3 clean teams
  await db.query(
    "INSERT INTO team (team_name) VALUES ('IT Support'), ('HR'), ('Finance')",
  );
  ...
  console.log(
    `Verification — notifications:${n} issues:${i} employees:${e} admins:${a}`,
  );
